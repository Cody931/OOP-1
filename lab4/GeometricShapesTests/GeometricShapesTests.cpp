...
BOOST_AUTO_TEST_SUITE(Point)
	...
	{
		boost::optional <CPoint> point = CPoint(0, 0);
		BOOST_CHECK(point);
	}
...
BOOST_AUTO_TEST_SUITE(Circle)
	BOOST_AUTO_TEST_CASE(can_be_generated)
	{
		boost::optional <CCircle> circle = CCircle(0, 0, 10);
		BOOST_CHECK(circle);
	}
